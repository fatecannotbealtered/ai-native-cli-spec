...
- [ ] A bare `update` (single command, no confirm token) syncs every Skill directory under `skills/` or returns a `skill_sync_command` equivalent to `npx skills add {{REPO_SLUG}} -y -g`; failures/interruptions report `stage` + `current_version` + `binary_replaced` + `skill_sync_status`.

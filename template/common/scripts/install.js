...
// postinstall downloader for the prebuilt {{TOOL_NAME}} binary.
// Language-agnostic: the release archive may hold a Go OR Python (e.g. PyInstaller) binary;
// this script only cares that bin/{{TOOL_NAME}}[.exe] ends up in place.
...
const SKIP = process.env["{{ENV_PREFIX}}_SKIP_INSTALL"] || process.env.SKIP_INSTALL;
const FORCE = process.env["{{ENV_PREFIX}}_FORCE_INSTALL"];
...
const REQUIRE_SIGNATURE = process.env["{{ENV_PREFIX}}_REQUIRE_SIGNATURE"] === "1";
...
if (SKIP) {
  console.log(`Skipping ${NAME} binary install ({{ENV_PREFIX}}_SKIP_INSTALL / SKIP_INSTALL set).`);
  process.exit(0);
}

if (fs.existsSync(dest) && !FORCE) {
  console.log(`${NAME} binary already present; skipping download (set {{ENV_PREFIX}}_FORCE_INSTALL=1 to redownload).`);
  process.exit(0);
}
